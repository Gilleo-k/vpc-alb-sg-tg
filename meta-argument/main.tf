...
resource "aws_iam_user" "us1" {
    name = "cloudadmin"
    depends_on = [ aws_instance.s1 ] //this mean a List, which mean you can have multiple element in there(means you can add more item in there.)
...
resource "aws_instance" "s2" {
    for_each =toset(var.typeofinstances)
    ami = "ami-045269a1f5c90a6a0"
